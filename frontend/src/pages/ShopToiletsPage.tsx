    ...
    <>
      <RevealSection className="bg-black px-6 pb-4 pt-20 text-center md:pt-28">
        <h1 className="text-4xl font-semibold text-white md:text-5xl">
          {subcategoryName || "View Toilets"}
        </h1>
        <p className="mx-auto mt-4 max-w-2xl text-lg text-white/70">
          Filter by shape, color, series, type, and extras to find the right fit.
        </p>
      </RevealSection>
            ...
            {filteredProducts.map((product) => {
              ...
              return (
                <div key={product.id} className="flex flex-col">
                  <Link
                    to={`/products/toilets/shop-toilets/${product.id}`}
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {colors.map((color, index) => (
                        <Link
                          ...
                          to={`/products/toilets/shop-toilets/${product.id}?color=${encodeURIComponent(color.name)}`}
                          title={color.name}
...
export default ShopToiletsPage;
